...
`resolveViewport()` 默认 1024×720。**一部分真实界面在这个宽度下，主操作面板整块不渲染**（2026-09-01 撞到的是一个交易页）。
于是探索采到的是一个缺了主功能的页面，而它看起来是完整的。
...
2026-09-01 实测：材料是整屏转储，模型把两个会自己变的读数（一个百分比、一个倒计时）原样
