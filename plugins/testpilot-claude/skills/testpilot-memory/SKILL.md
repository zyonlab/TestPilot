...
经验是否有效必须由本项目的有／无记忆实验判断，不引用外部论文的增益作为项目结论。

注册 workflow 的实际消费由 `load_run_instructions` 完成：只加载服务端从执行事实提取的固定模板，按项目、目标 URL、物料摘要和 pageVersion（默认物料摘要）隔离，最多 3 条，30 天过期。返回 memory.digest、来源 revision 与 memoryDigest binding；它证明下发，不证明注意力或收益。`TP_MEMORY_ENABLED=0` 或注册参数 `memoryEnabled=false` 可关闭，已有运行的消费快照保持不变。held-out 运行不读写记忆。自由 markdown 条目不会自动注入此链路。
