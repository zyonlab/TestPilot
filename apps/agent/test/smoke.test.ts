...
it("boots under the supervisor, answers health and beats", async () => {
  ...
  expect(await sup.rpc<ChildApi>("agent")!.health()).toMatchObject({ ok: true });
  expect(sup.statusOf("agent")!.lastHeartbeat).toBeDefined();
  expect(sup.statusOf("agent")!.currentTask).toBe("idle");
  expect(
    events.some(
