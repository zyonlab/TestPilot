...
TestPilot 拿**自己的规格**当材料，考自己的生成器。第二个基准，
主基准是 `../binance-futures/`。

它和主基准互补：主基准的材料是**观察**（只能发现「产品变了」），
这个基准的材料是**人写的文档**（`derivedFrom: "document"`，表达意图），
所以它是唯一能回答「用例有没有对着意图去测」的那一个。
...
任务书原计划让 `statement/` 把 `fixtures/mock-spec/acme-portal.md` 当材料引用。**没有这么做**，
理由是这两样东西对不上：
...
注意 `CAPABILITY` 默认是 `binance-futures`（`server/src/audit.ts:82`）：
要往这个基准写标注，得 `TP_CAPABILITY=casegen`，否则标注会落到主基准目录里去。
