...
function contextFor(b:Batch,u:Unit,deliver=false){
 ...
 const detail=store().getRun(b.runId,b.projectId).detail as any,env=resolveEnvironment(b.projectId,detail.target?.envRef);
 const target=caseEntryUrl(approved(b,u).precondition,resolveText(env?.baseUrl||detail.parameters?.sourceUrl||getProject(b.projectId)!.targetUrl,{env:env?.vars??{},secrets:getSecretValues(b.projectId)}));
 const exploration=selectExplorationContext(store(),b.runId,b.projectId,target,caseStartsLoggedOut(approved(b,u).precondition),dispatchedEnvironment(env,getSecretValues(b.projectId),detail.parameters?.exploreWallet===true));
 const payload={...selection,exploration,delivered:deliver,lessons,facts,factsReceipt:deliver?latestReceipt:undefined,factsNotice:'Last observed runner facts; a new browser must verify them again.'};
...
export async function preparationStep(runId:string,projectId:string,raw:unknown){
 ...
 if(input.action==='probe'){
  ...
  const plan={...original,lifecycle:undefined,steps:[],postSteps:[],expected:'',oracle:undefined,assertions:[]};
 ...
 if(original.lifecycle && (canonicalJSON(original.steps)!==canonicalJSON(plan.steps)||canonicalJSON(original.postSteps)!==canonicalJSON(plan.postSteps)))throw new LedgerError(409,'lifecycle_action_bindings_frozen');
...
function recordFailure(b:Batch,u:Unit,e:any,probe=false){const current=latest(b.runId,b.projectId);if(!current||current.id!==b.id||current.generation!==b.generation||current.status!=='running')return;const item=current.units.find(v=>v.caseId===u.caseId)!;item.status=probe?'planning':'blocked';item.reason=String(e.message).slice(0,1500);item.experienceContext=undefined;const prior=probe?item.probeResult:item.result;const priorReceipt=prior?store().readRevision(prior,b.projectId).content as any:null;const observation=readExecutionObservation(priorReceipt?.observation);const ref=put(current,`${u.caseId}/${probe?'probe-error':'error'}/${randomUUID()}`,{error:item.reason,infraError:true,lifecycle:priorReceipt?.lifecycle??unavailableLifecycle(),observation},[probe?u.probePlan!:u.plan!]).id;if(probe)item.probeResult=ref;else item.result=ref;save(current);log(current,`${u.caseId} · ${item.reason}`);}
...
async function trial(b:Batch,u:Unit,plan:TextCase,probe=false,probePreparation?:Preparation){
 ...
 try{
  ...
  const missing=[...login,...setup,...plan.steps,...plan.postSteps].flatMap(text=>{const keys=referencedKeys(text);return [...keys.env.filter(k=>k!=='TP_LIFECYCLE_ID'&&!(k in context.env)).map(k=>'env.'+k),...keys.secret.filter(k=>!(k in context.secrets)).map(k=>'secret.'+k)];});if(missing.length)throw new LedgerError(409,'unresolved_placeholders:'+missing.join(','));
