...
import { DOMAIN_PERP } from '../../packages/harness-testing/src/casegen/prompts.js';
...
try {
 for(let repetition=0;repetition<repetitions;repetition++) for(const arm of repetition%2?['on','off']:['off','on']) {
  ...
  const recorder={chat:async(req:ChatRequest)=>{
   const hasDomain=req.stable.endsWith(DOMAIN_PERP);if(hasDomain!==(arm==='on'))throw new Error('ablation_not_applied');
   ...
   const stable=req.stable.slice(0,hasDomain?-DOMAIN_PERP.length:undefined)+common+(hasDomain?DOMAIN_PERP+`\nCONTEXT_CANARY: ${canary}\n`:'');
