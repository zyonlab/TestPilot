...
export const CASES_STABLE = [
  ...
  "- The specification may list FLOWS with the steps that were actually walked. When this",
  "  case exercises one of those steps, put the transitions it goes through in `covers`,",
  "  written exactly as the specification writes them (`from->to`). A case that changes",
  "  nothing has nothing to put there — and that is worth noticing: it is checking that a",
  "  screen still looks the same, not that the product still does something.",
