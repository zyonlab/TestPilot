...
export function isInfraError(msg: string): boolean {
  return /EXEC_CANCELLED|ENV_RESET_FAILED|ENV_TEARDOWN_FAILED|BUDGET_EXHAUSTED/.test(msg) || INFRA.test(msg);
}

export function classifyFailure(message: string): Failure {
  const msg = message ?? "";
  const stopped = /EXEC_CANCELLED|ENV_RESET_FAILED|ENV_TEARDOWN_FAILED|BUDGET_EXHAUSTED/.exec(msg)?.[0];
