...
/**
 * 规则包与 charter 的硬事实校验（docs/v3/21 §8 反例 1、8、10 的服务端半边）。
...
/** `fill`：探索能填一个**规则包声明过的**值，不自己编（docs/v3/24 §17）。 */
describe("fill 目标", () => {
  const base = load() as Record<string, any>;
...
/**
 * 沙箱放开的是「可以改状态」，不是「点错也无所谓」（docs/v3/24 §17）。
...
/**
 * `match.within`：同一串文案在页面上出现几十次，只有弹窗里的那一个是要点的（docs/v3/24 §24.1）。
 *
...
/**
 * 同一个文案被两个会改状态的目标认领 = 两个一起点不动（docs/v3/24 §24.2）。
 *
...
/**
 * 链要**整条**能跑完才开始走，不能只看下一跳（docs/v3/24 §24.7）。
 *
...
/**
 * 下一跳还没出现在屏幕上时，供给方也要等（docs/v3/24 §24.7）。
 *
...
/**
 * 角色与生命周期是**包里的领域事实**，不是模型的发挥空间（docs/v3/24 §26）。
 *
