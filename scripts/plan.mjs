...
function parse(text) {
  ...
  lines.forEach((line, i) => {
    ...
    if (dep) cur.deps = (dep[1].match(/[A-Z]-\d+/g) ?? []);
...
const title = (raw.match(/^# (.+)$/m)?.[1] ?? "任务与进度").replace(/ — .*$/, "");
const docFlag = docArg >= 0 ? ` --doc ${DOC.replace(ROOT + "/", "")}` : "";
console.log(`\n${title} · ${DOC.replace(ROOT + "/", "")}\n`);
...
console.log(`\n  下一步：node scripts/plan.mjs${docFlag} --next\n`);
