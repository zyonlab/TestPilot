  ...
  return (
    <div className="rounded-md border border-border">
      <div className="flex items-center gap-1.5 border-b border-border px-2 py-1.5">
        ...
        <span className="text-[0.75rem] font-medium">{t("wf.materials")}</span>
        <span className="text-[0.6875rem] text-muted-foreground">
          {t("wf.materialsChosen").replace("{n}", String(selected.length))}
        </span>
      </div>
      {error && <div className="px-2 py-1.5 text-[0.75rem] text-bad">{error}</div>}
      <div className="max-h-48 overflow-auto">
        {missing.map((p) => (
          <label key={p} className="flex cursor-pointer items-center gap-2 px-2 py-1 hover:bg-muted">
            ...
            <span className="min-w-0 flex-1 truncate font-mono text-[0.6875rem] text-warn">{p}</span>
            <span className="shrink-0 text-[0.6875rem] text-warn">{t("wf.materialsMissing")}</span>
          </label>
        ))}
        {files.map((f) => (
          <label
          ...
          >
            ...
            <span className="min-w-0 flex-1 truncate font-mono text-[0.6875rem]">{f.path}</span>
      ...
      <p className="border-t border-border px-2 py-1 text-[0.6875rem] leading-relaxed text-muted-foreground">
        {t("wf.materialsOrder")}
      </p>
    </div>
  );
