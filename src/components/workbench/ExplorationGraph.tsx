...
export function ExplorationGraph({graph}:{graph:ExplorationGraphData}) {
  ...
  return <section className="space-y-3"><h3 className="font-medium">{t('exploration.graph')} · {states.length}</h3>
    ...
    {state&&<section className="space-y-3 rounded border border-border p-4 text-sm" aria-label={t('exploration.diff')}>
      ...
      {transition&&<><p className="break-all font-mono text-xs text-muted-foreground">{`sfg:edge:${transitionIndex}`} · {`sfg:state:${transition.from}`} → {transition.to?`sfg:state:${transition.to}`:'—'}</p><p className="font-medium">{label(transition.from)} → {label(transition.to)} · {transition.action.kind}: {transition.action.target}</p><p className="text-xs text-muted-foreground">{summary(transition)}</p>
