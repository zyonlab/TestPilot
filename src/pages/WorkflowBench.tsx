import {artifactLabel} from '@/components/workbench/artifactLabel';
import { WorkflowProgressDock } from '@/components/workbench/WorkflowProgressDock';
import { NodeDetail } from '@/components/workbench/NodeDetail';
import { useCallback, useEffect, useState } from 'react';
import { Play, Plus, Square, Circle, CheckCircle2, AlertCircle } from 'lucide-react';
...
function Bench({projectId}:{projectId:string}) {
  ...
  async function action(path:string,body:unknown={}){if(!run)return;setBusy(true);if(path==='resume')setResuming(true);setActionError('');try{await workflowRequest<{checkpoint?:{next:string}}>(`${workflowBase(projectId)}/${run.id}/${path}`,body);if(path==='resume'){setDetails(false);navigateProject('canvas',{nodeId:'',revisionId:''});}refresh();return true;}catch(e){setActionError(e instanceof Error?e.message:'request_failed');return false;}finally{setBusy(false);if(path==='resume')setResuming(false);}}
  /**
   * 「继续运行」什么时候该出现。
   *
  ...
  const stoppable=run&&['running','queued','pending'].includes(run.status);
  async function stop(){setStopping(true);try{await action('cancel');}finally{setStopping(false);}}
  ...
  return <main className="flex min-h-0 flex-1 flex-col" data-testid="workflow-bench"><header className="flex h-14 shrink-0 items-center justify-between border-b border-border bg-card px-5"><h1 className="text-base font-semibold">{t('bench.title')}</h1><Button size="sm" onClick={()=>navigateProject('settings',{projectId})}>{t('nav.settings')}</Button></header>
    ...
    {creating?<div className="min-h-0 flex-1 overflow-auto"><NewRunForm projectId={projectId} onClose={()=>setCreating(false)} onCreated={id=>{choose(id);refresh();}}/></div>:!run?<div className="grid flex-1 place-content-center gap-4 px-6 text-center"><h2 className="text-xl font-medium">{!loaded?t('workflow.loading'):ctx.runId?t('workflow.runMissing'):t('bench.empty')}</h2><p className="text-sm text-muted-foreground">{t('bench.emptyHint')}</p><Button variant="primary" onClick={()=>setCreating(true)}>{t('workflow.new')}</Button></div>:<div className="relative flex min-h-0 flex-1 flex-col">
      ...
      <Drawer open={!!(details||revision||ctx.nodeId)} onClose={closeDrawer} resizeKey={revision?'bench-artifact':ctx.nodeId&&!details?'bench-node':'bench-run'} defaultWidth={1080}
        title={revision?`${artifactLabel(revision.name,t)} · v${revision.revision}`:ctx.nodeId&&!details?t(ctx.nodeId==='source'?`bench.source.${run.detail?.parameters?.sourceKind??'spec'}`:`workflow.stage.${ctx.nodeId}`):t('bench.runDetails')}>
        <div className="p-5">{revision?<RevisionViewer hideTitle key={revision.id} projectId={projectId} revision={revision} all={run.revisions} onSelect={r=>navigateProject('canvas',{revisionId:r.id})}/>:ctx.nodeId&&!details?<NodeDetail projectId={projectId} run={run} node={ctx.nodeId} refresh={refresh} onRunDetails={()=>setDetails(true)}/>:<RunDetail projectId={projectId} run={run} refresh={refresh}/>}</div>
      </Drawer>
    </div>}
  </main>;
}
